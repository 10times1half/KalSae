    ...
    struct KSLinuxPALStubContractTests {
        ...
        func acceleratorParserMapsTokens() async {
            #expect(
                KSLinuxAcceleratorBackend.toGtkTrigger("Ctrl+Shift+K")
                    == "<Control><Shift>k")
            #expect(
                KSLinuxAcceleratorBackend.toGtkTrigger("CmdOrCtrl+N")
                    == "<Control>n")
            #expect(
                KSLinuxAcceleratorBackend.toGtkTrigger("Alt+F4")
                    == "<Alt>F4")
            #expect(
                KSLinuxAcceleratorBackend.toGtkTrigger("F11")
                    == "F11")
            #expect(
                KSLinuxAcceleratorBackend.toGtkTrigger("garbage+plus+thing")
                    == nil)
        }
