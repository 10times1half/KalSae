...
> ⚠️ **Experimental.** APIs may change. All five targets (Windows, macOS, Linux, iOS, Android) are stable; Linux has some platform limitations (for example global hot-keys on Wayland and tray behavior on vanilla GNOME).
...
> ⚠️ **실험적 단계입니다.** API는 변경될 수 있으며, Windows·macOS·iOS·Android가 가장 완성도가 높습니다. Linux는 기능 구현이 거의 완료되었지만(예: Wayland 글로벌 단축키, 순수 GNOME 트레이 동작 등) 플랫폼 제약이 일부 남아 있습니다.
...
| **Linux** (GTK4 + WebKitGTK 6.0) | Stable | Full PAL with platform caveats (global hot-keys on Wayland, tray behavior on vanilla GNOME) |
| **iOS** (UIKit + WKWebView) | Stable | Full PAL via UIKit (dialogs via UIDocumentPicker/UIAlertController, menus as no-op + `UIAlertController` action sheet for context menus, notifications via UNNotification); `run()` is host-driven — the UIKit `@main` entry point owns the lifecycle, so use `KSApp.boot()` + `KSiOSDemoHost` |
| **Android** (JNI + Android WebView) | Stable | Full PAL via JNI bridge (dialogs, menus via PopupMenu, notifications, clipboard, deep link); `run()` is host-driven — the Kotlin Activity owns the lifecycle, so use `KSApp.boot()` + `KSAndroidDemoHost` |
...
| **Linux** (GTK4 + WebKitGTK 6.0) | 안정 | 전체 PAL 구현, 플랫폼 제약 일부 존재 (Wayland 글로벌 단축키, 순수 GNOME 트레이) |
| **iOS** (UIKit + WKWebView) | 안정 | UIKit 기반 풍 PAL (UIDocumentPicker/UIAlertController 다이얼로그, 메뉴는 no-op + 컨텍스트는 actionSheet, UNNotification 알림); `run()` 은 호스트 구동 — UIKit `@main` 이 라이프사이클을 소유하므로 `KSApp.boot()` + `KSiOSDemoHost` 사용 |
| **Android** (JNI + Android WebView) | 안정 | JNI 브리지로 전체 PAL 제공 (다이얼로그, `PopupMenu` 기반 컨텍스트 메뉴, 알림, 클립보드, 딥링크). `run()`은 호스트 주도 — Kotlin Activity 가 라이프사이클을 소유하므로 `KSApp.boot()` + `KSAndroidDemoHost` 를 사용한다 |
...
- `commandAllowlist` — JS에서 호출 가능한 명령 화이트리스트 (`null`/`[]` 이면 사용자 명령 전체 차단 = **default-deny**; 내장 `__ks.*` 명령은 항상 허용). 등록된 모든 user 명령을 허용하려면 `commandAllowlistAll: true`로 opt-in
...
| `__KS_.window` | Window state, geometry, theme, multi-window, taskbar (35+ methods) | `await __KS_.window.toggleMaximize()` |
...
| `__KS_.secret` | OS-secure credential store (Keychain / Credential Manager) | `await __KS_.secret.set({ service: "api", account: "token" }, "sk-…")` |
...
| `__KS_.window` | 윈도우 상태/기하/테마/다중윈도우/작업표시줄 (35개 이상) | `await __KS_.window.toggleMaximize()` |
...
| `__KS_.secret` | OS 보안 자격증명 저장소 (Keychain / Credential Manager) | `await __KS_.secret.set({ service: "api", account: "token" }, "sk-…")` |
...
| Domain | Count | Sample |
|---|---|---|
| `__ks.window.*` | 35+ | `minimize`, `setSize`, `setTheme`, `setAlwaysOnTop`, `reload`, `setZoom`, `capturePreview`, `setOverlayIcon`, `create`, `list`, … |
...
| `__ks.dialog.*` | 4 | `message`, `openFile`, `saveFile`, `selectFolder` |
...
| `__ks.secret.*` | 4 | `set`, `get`, `delete`, `list` (gated by `security.secret`; service is auto-prefixed with `app.identifier`) |
| `__ks.app.quit` + `__ks.environment` + `__ks.log` | 3 | `quit`, `environment`, `log` |
...
| Credential store (`__ks.secret.*`) | ✅ Credential Manager | ✅ Keychain | ✅ libsecret⁶ | ✅ Keychain | ❌⁶ |
...
⁴ Linux: implemented via D-Bus StatusNotifierItem + DBusMenu (no AppIndicator3 / libayatana dependency). Works on KDE Plasma, Cinnamon, XFCE, Pantheon, and GNOME with the AppIndicator extension. Vanilla GNOME has no native SNI support — install logs a warning and falls back to no-op.

⁶ Linux uses libsecret (Secret Service via D-Bus — GNOME Keyring / KWallet / KeePassXC) and requires `libsecret-1-dev` at build time; on hosts without a Secret Service daemon the IPC commands return `.unsupportedPlatform`. The Android credential backend (AndroidKeyStore) is planned but not yet implemented. macOS/iOS/Windows/Linux all require the app to opt in via `security.secret.enabled = true`.
...
| 파일시스템 (`__ks.fs.*`) | ✅ | ✅ | ✅ | ✅ | ✅ |
| HTTP fetch (`__ks.http.fetch`) | ✅ | ✅ | ✅ | ✅ | ✅ |
| 자격증명 저장소 (`__ks.secret.*`) | ✅ Credential Manager | ✅ Keychain | ✅ libsecret⁶ | ✅ Keychain | ❌⁶ |
| 윈도우 상태 영속화 | ✅ | ✅ | ✅³ | ❌ | ❌ |
...
⁴ Linux는 D-Bus StatusNotifierItem + DBusMenu 직접 구현(AppIndicator3/libayatana 도입 없음). KDE Plasma, Cinnamon, XFCE, Pantheon 및 AppIndicator extension이 활성화된 GNOME에서 동작. 순수 GNOME은 SNI 다이젝트 지원이 없어 install이 경고 로그만 남기고 no-op으로 폴백.

⁶ Linux는 libsecret(Secret Service D-Bus — GNOME Keyring / KWallet / KeePassXC)를 사용하며 빌드 시 `libsecret-1-dev`가 필요하다. Secret Service 데몬이 없는 호스트에서는 IPC 시 `.unsupportedPlatform`을 반환한다. Android 백엔드(AndroidKeyStore)는 계획되어 있지만 미구현. macOS/iOS/Windows/Linux 모두 `security.secret.enabled = true` 로 명시적 옷인이 필요하다.
...
- **Command allowlist** — `security.commandAllowlist` selects which user commands JS can invoke. `null` or `[]` means **deny-all** (built-in `__ks.*` commands always work). Set `commandAllowlistAll: true` to allow every registered user command (not recommended for production).
...
- **명령 화이트리스트** — `security.commandAllowlist`로 JS에서 호출 가능한 user 명령을 제한합니다. `null`/`[]` 이면 **사용자 명령 전체 차단** (default-deny). 내장 `__ks.*` 명령은 항상 동작합니다. 등록된 모든 user 명령을 허용하려면 `commandAllowlistAll: true` 로 opt-in (프로덕션 비권장).
