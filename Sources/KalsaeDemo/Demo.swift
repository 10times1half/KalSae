...
struct PingOut: Codable, Sendable { let pong: Bool; let at: TimeInterval }
...
struct Demo {
    ...
    static func run() async throws {
        ...
        await app.registry.register("app.showInfo") { [app] _ in
            app.showMessage(KSMessageOptions(
                kind: .info,
                title: "Kalsae",
        ...
        await app.registry.register("app.openFile") { [app] _ in
            app.openFile(KSOpenFileOptions(
                title: "Pick a file",
                filters: [
        ...
        let tickTask = Task.detached { [weak app] in
            struct Tick: Encodable { let n: Int; let at: TimeInterval }
            var n = 0
    ...
    static func installMenus(app: KSApp) async throws {
        let fileMenu = KSMenuItem.submenu(id: "file", label: "File", items: [
            ...
            .action(id: "notify", label: "Show Notification", accelerator: "CmdOrCtrl+N", command: "app.notify"),
            .separator(),
            .action(id: "quit",   label: "Quit",              accelerator: "Alt+F4",      command: "app.quit"),
        ])
        let helpMenu = KSMenuItem.submenu(id: "help", label: "Help", items: [
