...
/// 모든 프레임에 주입되는 JavaScript. macOS 플랫폼의 `KSRuntimeJS`를
/// 미러링 — `webkit.messageHandlers` API는 iOS에서 동일하다.
...
public final class KSiOSWebViewHost {
    ...
    public func openDevTools() throws(KSError) {
        // iOS에서 Web Inspector를 사용하려면 macOS의 Safari + 기기 신뢰가 필요하다 — no-op.
    }
}

// MARK: - KSWebViewBackend 준수
...
extension KSiOSWebViewHost: KSWebViewBackend {
    ...
    public func openDevTools() async throws(KSError) {
        // iOS에서 Web Inspector는 공개 API로 사용할 수 없다.
    }
}
