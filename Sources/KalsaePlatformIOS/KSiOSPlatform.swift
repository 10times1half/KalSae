...
public final class KSiOSPlatform: KSPlatform, @unchecked Sendable {
    ...
    private func runOnMain(
        ...
        _deepLink = deepLinkPair?.backend as? KSiOSDeepLinkBackend
        ...
        await KSBuiltinCommands.register(
            ...
            quit: { /* iOS has no programmatic quit API */ },
            platformName: name,
        ...
        #endif
        try host.start(url: Self.resolveStartURL(
            windowURL: window.url,
            devServerURL: config.build.devServerURL,
...
private extension KSiOSPlatform {
    enum ServingMode {
