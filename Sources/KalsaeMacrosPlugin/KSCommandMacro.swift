...
public struct KSCommandMacro: PeerMacro {
    public static func expansion(
        ...
        // `permission:` 레이블 인자를 추출 (제공되면).
        let permission = Self.permissionArgument(from: node)
        ...
        let funcDecl: String
        if let permission {
            // permission 리터럴은 validateAttributeArguments가 이미 이스케이프가
            // 들어 있지 않음을 확인했으므로 그대로 섬입한다.
            funcDecl = """
                ...
                await registry.setMetadata("\(registryName)", permission: "\(permission)")
                }
                """
        } else {
            funcDecl = """
    ...
    private static func registryName(from node: AttributeSyntax) -> String? {
        ...
        // 첫 번째 *라벨 없는* 인자를 이름으로 본다.
        guard let first = args.first(where: { $0.label == nil }) else { return nil }
    ...
    /// `@KSCommand(..., permission: "fs:read")`에서 권한 문자열을 추출한다.
    /// 없거나 리터럴이 아니면 nil. 빈 문자열도 nil로 반환.
    private static func permissionArgument(from node: AttributeSyntax) -> String? {
        guard case .argumentList(let args) = node.arguments else { return nil }
        guard let perm = args.first(where: { $0.label?.text == "permission" }) else {
            return nil
        }
        guard let literal = perm.expression.as(StringLiteralExprSyntax.self) else {
            return nil
        }
        var s = ""
        for seg in literal.segments {
    ...
    private static func validateAttributeArguments(
        ...
        // 라벨이 있는 인자는 `permission:`만 허용.
        for arg in args {
            if let label = arg.label?.text, label != "permission" {
                context.diagnose(
                    Diagnostic(
                        node: Syntax(arg),
                        message: KSCommandDiagnostic.unknownArgumentLabel))
                return false
            }
        }
        // 인자 위치별 분류.
        var nameArg: LabeledExprSyntax? = nil
        var permArg: LabeledExprSyntax? = nil
        for arg in args {
        ...
        if let perm = permArg {
            guard let literal = perm.expression.as(StringLiteralExprSyntax.self) else {
                context.diagnose(
                    Diagnostic(
                        node: Syntax(perm.expression),
                        message: KSCommandDiagnostic.nonLiteralPermission))
                return false
            }
            for seg in literal.segments {
                if seg.as(StringSegmentSyntax.self) == nil {
            ...
            if raw.isEmpty {
                context.diagnose(
                    Diagnostic(
                        node: Syntax(literal),
                        message: KSCommandDiagnostic.emptyPermission))
                return false
            }
        }
        return true
