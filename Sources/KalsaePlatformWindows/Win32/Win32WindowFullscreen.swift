...
extension Win32Window {
    ...
    func setFullscreen(_ enabled: Bool) {
        ...
        if enabled {
            ...
            let WS_OVERLAPPEDWINDOW_LP: LONG_PTR = LONG_PTR(WS_OVERLAPPEDWINDOW)
            _ = SetWindowLongPtrW(hwnd, GWL_STYLE, style & ~WS_OVERLAPPEDWINDOW_LP)
            _ = SetWindowPos(
                hwnd, nil,
                rc.left, rc.top,
            ...
            _ = SetWindowPos(
                hwnd, nil, 0, 0, 0, 0,
                UINT(SWP_NOMOVE) | UINT(SWP_NOSIZE) | UINT(SWP_NOZORDER) |
                UINT(SWP_NOOWNERZORDER) | UINT(SWP_FRAMECHANGED))
            savedFullscreenStyle = nil
        }
    }
}
