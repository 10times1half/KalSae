...
/// Win32 implementation of the deep-link feature. Persists per-user
/// associations under `HKCU\Software\Classes\<scheme>` so no UAC prompt
/// is required.
///
/// Registry shape (mirrors Tauri's `plugin-deep-link`):
///
/// ```text
...
public struct KSWindowsDeepLinkBackend: KSDeepLinkBackend, Sendable {
    ...
    public func extractURLs(
        ...
        for a in args {
            ...
            guard lowerSchemes.contains(scheme),
                  URL(string: a) != nil else { continue }
            out.append(a)
        }
        return out
    }
    ...
    private static func resolveModulePath() throws(KSError) -> String {
        ...
        while capacity <= 32768 {
            ...
            if Int(rc) == capacity { capacity *= 2; continue }
    ...
    private static func writeStringValue(
        ...
        let openHr = keyPath.withUTF16Pointer { sub -> LONG in
            ...
            return RegCreateKeyExW(
                ...
                DWORD(0x20006) /* KEY_WRITE */, nil, &hKey, &disposition)
        }
    ...
    private static func readStringValue(keyPath: String, valueName: String) -> String? {
        ...
        let openHr = keyPath.withUTF16Pointer { sub in
            RegOpenKeyExW(HKEY_CURRENT_USER, sub, 0, DWORD(0x20019) /* KEY_READ */, &hKey)
        }
