...
public enum KSWindowsSingleInstance {
    ...
    public static func acquire(
        ...
        if handle == nil {
            ...
            return installPrimaryReceiver(className: className,
    ...
    private static func installPrimaryReceiver(
        ...
        guard atom != 0 else {
            log.warning("RegisterClassExW failed for receiver class (GetLastError=\(GetLastError())); single-instance relay disabled")
            return .primary
        }

        // HWND_MESSAGE = -3. 사용자에게 보이지 않으면서 WM_COPYDATA를
        // 받을 수 있는 메시지 전용 윈도우를 생성한다.
        let HWND_MESSAGE = HWND(bitPattern: -3)
        let hwnd = className.withUTF16Pointer { namePtr -> HWND? in
            CreateWindowExW(0, namePtr, namePtr,
                            0, 0, 0, 0, 0,
    ...
    private static let receiverWndProc: @convention(c) (
    ...
    private static func relayArguments(args: [String],
                                       className: String,
                                       log: Logger) {
        ...
        payload.withUnsafeBufferPointer { buf in
            ...
            withUnsafeMutablePointer(to: &cds) { ptr in
                _ = SendMessageW(target, UINT(WM_COPYDATA), 0,
