...
private extension KSLinuxPlatform {
...
public final class KSLinuxDemoHost {
    ...
    public func setOnSuspend(_ cb: (@MainActor () -> Void)?) {
        ...
        if cb != nil {
            ...
            ks_gtk_host_set_on_suspend(webview.hostPtr,
    ...
    public func setOnResume(_ cb: (@MainActor () -> Void)?) {
        ...
        if cb != nil {
            let ctx = Unmanaged.passUnretained(powerBox!).toOpaque()
            ks_gtk_host_set_on_resume(webview.hostPtr,
...
private let linuxCloseHandlerTrampoline: @convention(c) (
    UnsafeMutableRawPointer?
) -> Int32 = { raw in
...
private let linuxSuspendTrampoline: @convention(c) (
    UnsafeMutableRawPointer?
) -> Void = { raw in
    guard let raw else { return }
...
private let linuxResumeTrampoline: @convention(c) (
    UnsafeMutableRawPointer?
) -> Void = { raw in
    guard let raw else { return }
    let box = Unmanaged<PowerBox>.fromOpaque(raw).takeUnretainedValue()
