...
public struct KSMacMenuBackend: KSMenuBackend, Sendable {
    ...
    public func installWindowMenu(_ handle: KSWindowHandle,
    ...
    public func showContextMenu(_ items: [KSMenuItem],
                                 at point: KSPoint,
                                 in handle: KSWindowHandle?) async throws(KSError) {
        await MainActor.run {
            let menu = buildMenu(items)
            let nsPoint = NSPoint(x: CGFloat(point.x), y: CGFloat(point.y))
            let nsWindow = handle.flatMap { KSMacHandleRegistry.shared.window(for: $0)?.nsWindow }
    ...
    private func resolveModifierFlags(_ accel: String) -> NSEvent.ModifierFlags {
        ...
        if lower.contains("cmdorctrl") || lower.contains("cmd") || lower.contains("command") || lower.contains("super") || lower.contains("meta") {
