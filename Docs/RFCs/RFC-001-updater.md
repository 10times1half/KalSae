...
Linux `__ks.window.startDrag` 네이티브 구현 (RFC-005 §4.6에서 이관) — `KSWindowState.startDrag` 프로토콜 및 등록은 완료, 데스크톱 3개 플랫폼 중 Windows/macOS만 네이티브 구현, Linux는 프로토콜 default no-op 사용. GTK4 `gdk_toplevel_begin_move` 호출이 필요하며, `Sources/CKalsaeGtk/`에 `ks_gtk_host_begin_move(GdkSurface*, GdkDevice*, button, x, y, timestamp)` C shim 추가 필요. mousedown 이벤트의 GdkEvent 타임스탬프/디바이스를 캡처해 전달하는 GtkGestureClick 연결도 필요해 별도 사이클로 분리.
