...
#### Issue #5: `KSBuiltinCommands+Window.swift` — `MainActor.run` 내 불필요한 `catch let e as KSError`
...
`KSWindowEmitHub.shared.emit()`은 `throws(KSError)`로 선언되어 있어, bare `catch`만으로도 `KSError`가 자동 바인딩된다. `catch let e as KSError` 패턴이 불필요하다.

**영향:**
코드 복잡성만 증가시키고 실제 버그는 아니다.
...
let r: Result<Void, KSError> = await MainActor.run {
    do {
        ...
        return .failure(error)  // 이미 KSError
    }
}
```
...
| P2 | #5 `MainActor.run` 내 불필요 패턴 | 🟡 Warning | bare `catch`로 단순화 |
...
| P8 | #2 부동소수점 정밀도 | 🔴 Critical | 표준 유틸리티 사용 |
| P9 | #3 `@unchecked Sendable` | 🔴 Critical | `Mutex` 마이그레이션 검토 |
